...
order_min = str2double(answer{1});
order_max = str2double(answer{2});
...
for r = 1:length(regimes)
    ...
    for i = 1:length(response_vars)
        ...
        if strcmp(regime, 'torque')
            ...
            sched_all = X_all.torque * 6;
        else
            excitation_all = X_all.torque * 6;
            sched_all = X_all.speed;
        end
        ...
        sched_all = trim_edges(sched_all, trim_sec, fs);
        ...
        for na = order_min:order_max
            try
                ...
                data_id = iddata(Y_valid, excitation_valid, 1/fs);
                Y_pred = predict(model, data_id, 1);
        ...
        for k = 1:hop_size:(N - segment_length + 1)
            ...
            input_vector = [mean(sched_train(idx)), mean(torque_train(idx))];
            all_inputs(end+1, :) = input_vector;
            all_mu(end+1) = mean(residuals_train(idx));
            all_sigma2(end+1) = var(residuals_train(idx)) + 1e-6;
        end
