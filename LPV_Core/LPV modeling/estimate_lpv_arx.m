...
switch options.estimator.type
    ...
    case 'ols'
        M_tmp = ols(Phi, y(tau));
        theta = M_tmp.Parameters.Theta; % Get correct parameter vector
        
        M.a = reshape(theta(1:na*pa), pa, na);
        M.b = reshape(theta(na*pa+1:end), pa, nb+1);
        
        % Keep other useful information
        M.Parameters = M_tmp.Parameters;
        M.ParameterVector = theta; % <<< ADD THIS LINE
        M.Innovations = M_tmp.Innovations;
        M.InnovationsVariance = M_tmp.InnovationsVariance; % <<< ADD THIS
        M.Performance = M_tmp.Performance;
